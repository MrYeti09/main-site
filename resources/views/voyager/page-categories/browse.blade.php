    ...
    <div class="page-content browse container-fluid">
        ...
        <div class="row">
                            ...
                            <div class="form-group" style="margin-bottom:0px;">
                                <select class="form-control" name="s">
                                    ...
                                    @foreach($fields as $key => $field)
                                        @if(isset($field['name']['name']))
                                        <option @if(request()->has('s') and isset($field['name']['id']) and request('s') == $field['name']['id']) selected="selected" @endif value="{{$field['name']['id']}}">{{$field['name']['name']}}</option>
                                        @elseif(isset($field['name']))
                                        <option @if(request()->has('s') and isset($field['key']) and request('s') == $field['key']) selected="selected" @endif value="{{$field['key']}}">{{$field['name']}}</option>
                                        @endif
            ...
            @foreach($dataTypeContent as $pageCategoryKey => $pageCategory)
                ...
                @if(((!request()->has('s') or (request('s') == $pageCategory->id or request('s') == $pageCategory->name)) or request('s') == ""))
                    <div class="col-md-12">
                        <div class="panel panel-bordered">
                            <div class="panel-body">
                                ...
                                @endforeach
                                    ...
                                    <a class="btn btn-via" style="margin-right:3px;background:#0f447a;padding: 4px 12px;color:white;" href="{{route('voyager.page-categories.show',['id' => $pageCategory->id])}}">Informações</a>
                                        <a class="btn btn-via" style="margin-right:3px;background:#0f447a;padding: 4px 12px;color:white;" href="{{route('voyager.page-categories.edit',['id' => $pageCategory->id])}}">Editar</a>
                                    <a class="btn btn-via" style="margin-right:3px;background:#0f447a;padding: 4px 12px;color:white;" href="{{route('voyager.page-categories.blocks.view',['id' => $pageCategory->id])}}">Blocos</a>
                                        <form action="{{route('voyager.page-categories.destroy',['page_category' => $pageCategory->id])}}" method="post">
                                            <input type="hidden" name="_method" value="delete" />
                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <button class="btn btn-via" style="background:#0f447a;padding: 4px 12px;color:white;" href="">Deletar</button>
                                        </form>
                                    </div>
                                </div>
                                <hr>
