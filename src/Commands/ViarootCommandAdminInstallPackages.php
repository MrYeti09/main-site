...
class ViarootCommandAdminInstallPackages extends Command
{
     ...
     * @var string
     */
    protected $signature = 'viaativa-site:admin-packages';
    ...
    {
        $file_lines = file(base_path() . '/resources/assets/sass/viaativa-admin/admin.scss');
        $lines = [];
        foreach ($file_lines as $line) {
            array_push($lines, str_replace(['@import "', '.scss";', "\n"], "", $line));
        }

        if (!in_array($name, $lines) and $name != "admin.scss") {

            $fp = fopen(base_path() . '/resources/assets/sass/viaativa-admin/admin.scss', 'a');//opens file in append mode
    ...
    {
        $dirs = scandir(base_path() . '/resources/assets/sass/viaativa-admin');
        unset($dirs[0]);
        unset($dirs[1]);
        foreach ($dirs as $dir) {
            $this->addSass($dir);
