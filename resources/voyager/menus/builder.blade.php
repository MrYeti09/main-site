            ...
            <div class="modal-content">
                ...
                <form action="" id="m_form" method="POST"
                    ...
                    <div class="modal-body">
                        ...
                        <div id="">
                            <label for="m_desc">Descrição</label><br>
                            <textarea id="m_desc" class="form-control" name="description" row="4"></textarea>
                        </div><br>
                        ...
                        <div id="m_img">
                            <label for="m_img">Imagem</label>
                            <input style="min-height:78px;" type="file" accept="image/*" class="form-control" id="m_img" name="img" placeholder=""><br>
                        </div>
                        <div id="m_img_hover">
                            <label for="m_img_hover">Imagem (hover)</label>
                            <input style="min-height:78px;" type="file" accept="image/*" class="form-control" id="m_img_hover" name="img_hover" placeholder=""><br>
                        </div>

                        <label for="permissions">Permissões</label>
                            <select id="m_permissions" class="form-control select2" multiple name="permissions[]">

                                @foreach(\TCG\Voyager\Models\Role::all() as $key => $role)
    ...
    <script>
        $(document).ready(function () {
            ...
            $m_modal.on('show.bs.modal', function(e, data) {
                ...
                if (_adding) {
                    ...
                    var _src = e.relatedTarget.data, // the source
                        id   = _src.data('id');
                    //console.log(_src.data('desc'));
                    $m_desc.val(_src.data('desc'));
                    ...
                    $m_desc.val(_src.data('desc'));
                    $m_id.val(id);
                    console.log(_src.data('permissions'))
                    var $selected_permissions = _src.data('permissions');
                    $('#m_permissions option').each(function() {
                        if($selected_permissions != null) {
                            if ($selected_permissions.includes( parseInt($(this).val()) )) {
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('voyager.menus.order-faster',['menu' => $menu->id]) }}', {
