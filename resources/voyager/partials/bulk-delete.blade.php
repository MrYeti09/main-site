@if($dataType->slug != "page-categories")
    ...
    <div class="modal modal-danger fade" tabindex="-1" id="bulk_delete_modal" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('voyager.pages.index') }}/0" id="bulk_delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
                        <input type="text" name="ids" id="bulk_delete_input" value="">
                        <input type="submit" class="btn btn-danger pull-right delete-confirm"
                               value="{{ __('voyager::generic.bulk_delete_confirm') }} {{ strtolower($dataType->display_name_plural) }}">
                    </form>
                    <button type="button" class="btn btn-default pull-right" data-dismiss="modal">
        ...
        window.onload = function () {
            ...
            $bulkDeleteBtn.click(function () {
                ...
                if (count) {
                    ...
                    Swal.fire({
                        title: "ATENÇÃO!",
                        text: "Você está deletando as paginas selecionadas, digite 'DELETAR' para confirmar",
                        input: 'text',
                        type: 'warning',
                        ...
                        if (result.value) {
                            if (result.value == "DELETAR") {
                                toastr.warning('Deletando as páginas, por favor aguarde.')
