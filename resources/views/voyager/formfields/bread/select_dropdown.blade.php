...
@php
    //if(isset($child)) { dd(get_defined_vars()); }
    if(!is_object($options)) { $options = (object)$options; }

        if(property_exists($options,"link"))
        {
            if(!is_object($options->link))
            {
            $options->link = (object)$options->link;
            }
        $model = $options->link->model;
        if($dataType->model_name == $model)
        {
        $items = $model::where('id','<>',$dataTypeContent->id)->get();
...
@elseif(property_exists($options,"link"))
    ...
    if(!property_exists($options->link,"name"))
        {
        $options->link->name = 'id';
        }
    ...
    <select class="form-control select2" name="{{ $row->field }}">
        ...
        <option value="" @if($dataTypeContent->{$row->field} == null){{ 'selected="selected"' }}@endif>Nenhum</option>
            @foreach($items as $key => $val)
                <option value="{{ $val->{$options->link->name} }}"  @if($selected_value == $val->{$options->link->name}){{ 'selected="selected"' }}@endif>{{ $val->{$options->link->display} }}</option>
            @endforeach
    </select>
@endif
