...
@section('content')
    <div class="page-content browse container-fluid">
        ...
        <div class="row">
            @php
                $register = \Pvtl\VoyagerForms\Form::where('hook','register')->first();
            @endphp
            @if($register != null)
                <div class="col-md-2">
                    <div class="panel panel-primary">
                        <div class="panel-heading" style="padding:1px;">
                            <h3 style="margin: 10px;">Principais</h3>
                        </div>
                        <div class="panel-body" style="padding-top:15px;">
                            <a class="btn btn-primary" href="{{ route('voyager.'.$dataType->slug.'.edit', $register->{$register->getKeyName()}) }}">
            ...
            @endif
            <div class="@if($register != null) col-md-10 @else col-md-12 @endif ">
                        ...
                        <div class="table-responsive">
                            <table id="dataTable" class="table table-hover">
                                ...
                                @foreach($dataTypeContent as $data)
                                    @if($data->hook == "register") @continue @endif
                                    <tr>
                                        @can('delete',app($dataType->model_name))
                                            <td>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                <?php if(is_string($row->details)) {$options = json_decode($row->details); } else { $options = $row->details; } ?>
                                                @if($row->type == 'image')
                                        ...
                                        <td class="no-sort no-click" id="bread-actions">
                                            ...
                                            @endcan
                                            @can('edit', $data)
                                                <a style="background:#0f447a;padding: 4px 12px;" href="{{ route('voyager.'.$dataType->slug.'.edit', $data->{$data->getKeyName()}) }}" title="{{ __('voyager::generic.edit') }}" class="btn btn-sm btn-primary pull-right edit">
                                                    <i class="voyager-edit" style="color:white;font-size:16px;"></i>
                                                </a>
                                            @endcan
                                            @can('read', $data)
                                                <a style="background:#0f447a;padding: 4px 12px;" href="{{ route('voyager.'.$dataType->slug.'.show', $data->{$data->getKeyName()}) }}" title="{{ __('voyager::generic.view') }}" class="btn btn-sm btn-warning pull-right">
                                                    <i class="voyager-eye" style="color:white;font-size:16px;"></i>
                                                </a>
                                            @endcan
                                            @can('edit', $data)
                                                <a style="background:#0f447a;padding: 4px 12px;margin-right:6px;" href="{{ route('voyager.enquiries.index') }}?key=id&filter=contains&s={{ $data->{$data->getKeyName()} }}" title="Ver respostas" class="btn btn-sm btn-warning pull-right">
                                                    <i class="voyager-receipt" style="color:white;font-size:16px;"></i>
                                                </a>
                                            @endcan
                                        </td>
                                    </tr>
