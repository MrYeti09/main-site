<div class="icons-wrapper icons-wrapper-{{ $row->field }}">
    <div class="icon-selected">
        <input type="hidden" name="{{ $row->field }}" @if($row->required == 1) required @endif @if(is_object($dataTypeContent)) value="{{ $dataTypeContent->{$row->field} }}" @endif>
        <i class="icon-preview @if(is_object($dataTypeContent)) {{$dataTypeContent->{$row->field} }} @endif"  style="font-size: 80px;"></i>
    </div>
