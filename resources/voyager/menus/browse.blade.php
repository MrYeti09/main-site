                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="no-sort no-click bread-actions">
                                        @can('delete', $data)
                                            <div class="btn btn-sm btn-danger pull-right delete" title="Deletar" style="background:#0f447a;padding: 4px 12px;" data-id="{{ $data->{$data->getKeyName()} }}">
                                                <i class="voyager-trash" style="color:white;font-size:16px;" ></i>
                                            </div>
                                        @endcan
                                        @can('edit', $data)
                                            <a style="background:#0f447a;padding: 4px 12px;" title="Editar" href="{{ route('voyager.'.$dataType->slug.'.edit', $data->{$data->getKeyName()}) }}" class="btn btn-sm btn-primary pull-right edit">
                                                <i class="voyager-edit" style="color:white;font-size:16px;" ></i>
                                            </a>
                                        @endcan
                                        @can('edit', $data)
                                            <a style="background:#0f447a;padding: 4px 12px;" title="Construtor de Menus" href="{{ route('voyager.'.$dataType->slug.'.builder', $data->{$data->getKeyName()}) }}" class="btn btn-sm btn-success pull-right">
                                                <i class="voyager-list" style="color:white;font-size:16px;" ></i>
                                            </a>
                                        @endcan
                                    </td>
                                </tr>
