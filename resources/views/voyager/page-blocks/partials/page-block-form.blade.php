<div class="modal-header">
    ...
    <ul class="nav nav-tabs" id="tabContent">
        ...
        @foreach($clones as $key => $i)
                    <?php $tabsQnt += 1 ?>
            <li style="display:flex;margin-right:8px;"><a href="#tab{{$key+1+sizeof((array)$template->tabs)}}" data-toggle="tab">Item {{$i+1}}</a>
                <a href="{{route('voyager.page-blocks.remove-tab',['id' => $block->id,'tab_id' => $i])}}"><i class="fas fa-minus"></i></a></li>
        @endforeach
        @endif
        @if(isset($template->clonable))
                <?php $tabsQnt += 1 ?>
            <li><a href="{{route('voyager.page-blocks.add-tab',['id' => $block->id])}}"><i class="fas fa-plus"></i></a></li>
            ...
            @php
                foreach($template->fields as $key => $field)
                {
                    if(property_exists($field,'tab') and $field->tab == "responsive")
                    {
                    $template->fields->{$key}->tab = $tabsQnt;
                    }
                }
            @endphp
            <li style="float:right;margin-right:4px;"><a href="#tab{{$tabsQnt}}"  data-toggle="tab"><i style="font-size:23px;" class="fas fa-mobile-alt"></i></a></li>
    </ul>

<div class="modal-body">

    <div style="">
        <form role="form" class="form-save" action="{{ route('voyager.page-blocks.update', $block->id) }}" method="POST"
            ...
            <div class="row label-no-wrap">
                ...
                @foreach(collect($template->fields)->groupBy('tab') as $template_key => $template_group)
                    @if(isset($template_key))
                        ...
                        <div class="tab-pane @if($template_key == 1 || strlen($template_key) == 0) active @endif" id="tab{{$template_key}}">
                            @endif
                            @foreach($template_group as $key => $row)
                                @if($row->partial == 'break')
                                    <div class="col-md-12" style="height:1px;width:100%;background:#eeeeee;margin: 30px 10px;"></div>
                                    @continue
                                    @endif
                                @php $options = $row;
                ...
                @if(property_exists($template,'clonable') and property_exists($template,'fields') and (is_object($clones) or is_array($clones)))
                    ...
                    if(is_array($dataTypeContent)) { $dataTypeContent = (object)$dataTypeContent; }
                        ...
                        <div class="modal-footer" style="right:0;position: absolute;">
                            <button block-id="{{$block->id}}"
                                    data-save-block-btn
                                    style=""
                                    type="button"
                                    class="btn btn-success save save-this-block"
...
{{--                    </span>--}}
    ...
    <script>
        ...
        $('.save-this-block').click(function() {
            tinymce.triggerSave();
            var swals = Swal.fire({
                title: "Bloco salvo!",
            ...
            Swal.showLoading()
            var options = new FormData($('.form-save').get(0))
            ...
            $.ajax({
                ...
                error: function(data)
                {
                    Swal.hideLoading()
                    swals.close()
                    toastr.error('Erro desconhecido ao salvar.')
                    console.error(data)
                }
            })
        })
